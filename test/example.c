...
int main() {
    ...
    logger_save_to_page_line(logger, "Another one", 0, 1);

    logger_print_page(logger, 1, LOGGER_FLUSH);
    printf("Here start\n");
    logger_print_page(logger, 1, LOGGER_DEFAULT);
    printf("Here end\n");
    // logger_save_to_page(logger, "This is a test.", 15, 1);
    // printf("Saved data to page 1 and 0.\n");
    logger_print_page(logger, 0, LOGGER_DEFAULT);
